...
typedef struct {
    ...
    XCirc_buff_read_128_uio_map maps[ MAX_UIO_MAPS ];
} XCirc_buff_read_128_uio_info;

/***************** Variable Definitions **************************************/
static XCirc_buff_read_128_uio_info uio_info;
...
static int uio_info_read_name(XCirc_buff_read_128_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCirc_buff_read_128_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCirc_buff_read_128_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCirc_buff_read_128_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCirc_buff_read_128_Initialize(XCirc_buff_read_128 *InstancePtr, const char* InstanceName) {
	XCirc_buff_read_128_uio_info *InfoPtr = &uio_info;
